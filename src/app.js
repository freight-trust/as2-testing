...
function saveInputs(page) {
  ...
  for(var i = 0; i < inputs.length; i++) {
    ...
    if(inputs[i].type == "checkbox") name = inputs[i].id;
    
    if(page == "settings") settings[name] = flag;
    else advanced[name] = flag;
  }
...
function checkBeforeSave() {
  ...
  for (var i = 0; i < requiredinputs.length; i++) {
    ...
    if (requiredinputs[i].value.trim() == "") {
      if (state != "undefined") {
        state.removeClass("correct");
        state.addClass("error");
        state.text(lang("needRequiredInputs"));
      }
      color = "red";
      prepared = false;
    }
    $(".requiredInput:eq(" + i + ")").parent().next().css("color", color);
  }
  if (prepared) {
    if (typeof checkBeforeSaveApp == "function") {
      prepared = checkBeforeSaveApp();
    }
  }
  return prepared;
}
...
function checkUnsave(page, withoutConfirm, checkRadio){
  ...
  for(var i = 0; i < inputs.length; i++) {
    ...
    var name = inputs[i].name;
    if(inputs[i].type == "checkbox") name = inputs[i].id;
    var value = page == "advanced" ? advanced[name] : settings[name];
...
function showCert(certfile, showPrivate, isPaid) {
  if(isPaid) return false;

  $('#certfile_id').val(certfile);
  
  var url = "src/listCerts.rsb?showprivate=" + showPrivate + "&@json";
  var result = $.ajax({ 
    url: url,
    async: false
  }).responseText;

  $('#select_cert')[0].innerHTML = "";

  var certs = eval("(" + result + ")").items;

  $.each(certs, function(key, value) {
    $('#select_cert').append($('<option>', {value : value["fullname"]}).text(value["name"])); 
  });
...
// = select cert =========================
function selectCert() {
  var certFile = $('#certfile_id').val();
  var selectCert = $("#select_cert option:selected").text().trim();
  if (selectCert) {
    $(certFile).val(selectCert);
    $('#darkNight').hide();
    $('#addCertificate').hide();
  }
}
...
function checkCertExist(certFile) {
  var url = "src/checkCertExsit.rsb?certfile=" + certFile + "&@json&@fmtoptions=addparen";
  var result = $.ajax({ 
    url: url,
    async: false
  }).responseText;
  ...
  $(window).bind("sendorrestartqueuestart", function(event, url, id){
    ...
    $.get(url, function(msgData) {
      if(msgData && msgData.indexOf('class="correct"') < 0) {
        ...
        if (msgData.indexOf("Successful") < 0) {
          data += "<div>" + msgData + "</div>";
        } else {
          cItem++;
        }
      }
...
function openOAuthUrl() {
  var returl;
  var result = $.ajax({ 
    url: "src/getOAuthAuthorizationURL.rsb?@json&@fmtoptions=exparen", 
    async: false,
    dataType: 'json',                   
    success : function(data) {  
      errorMsg = data.items[0]["rsb:emessage"];
      if (errorMsg) {
        alert(errorMsg);
      } else {
        returl = data.items[0]["url"];
      }
    }
  }).responseText;
  
  if (returl) {
    var win = window.open(returl, '_blank');
    win.focus();
  }
}